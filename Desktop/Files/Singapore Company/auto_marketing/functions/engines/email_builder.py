...
class SMTPConfig:
    ...
    def from_env(cls, recipient_email: str | None = None) -> "SMTPConfig":
        ...
        from_email = os.getenv("SMTP_FROM_EMAIL", "").strip() or username or ""
        if not from_email:
...
<body>
  <div class="card" style="background:#0073b1;color:#fff;padding:16px 24px;">
    <h1 style="color:#fff;margin:0">Intonation Labs · Daily Brief</h1>
    <div style="color:#cce4f5;font-size:13px;margin-top:4px">{today} · {header_suffix}</div>
  </div>
  {post_html}
  {intel_html}
  {"".join(lead_sections)}
  <div class="footer">Generated by Intonation Labs Auto Marketing · {now_str} ({_escape(timezone_name)})</div>
</body>
</html>"""
