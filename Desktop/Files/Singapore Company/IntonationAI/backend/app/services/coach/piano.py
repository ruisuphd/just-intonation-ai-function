...
from app.services.llm.prompts import PIANO_COACH_PROMPT, PIANO_COACH_RAG_SECTION
...
class PianoCoach(BaseCoach):
    coach_type = "piano"

    async def _build_system_prompt(
        self, user_text: str, use_rag: bool = True
    ) -> str:
        query = f"piano technique {user_text}" if user_text else "piano pedagogy"
        context = (
            await retriever.retrieve_context(query, top_k=3)
            if use_rag
            else ""
        )
        rag_block = PIANO_COACH_RAG_SECTION.format(context=context) if context else ""
        return PIANO_COACH_PROMPT.format(rag_context=rag_block)
    ...
    async def get_welcome_message(self) -> str:
        return (
            "Hi! I'm your AI piano coach. Play into the mic and I'll analyse your notes, "
            "chords, timing, and technique to give you specific, actionable feedback. Let's go!"
        )
