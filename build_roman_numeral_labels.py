...
def _analyse_music21(score_path: str) -> List[Dict[str, Any]]:
    """Run music21's built-in Roman numeral analysis on a MusicXML score.

    Uses music21.analysis.floatingKey for local key estimation and
    music21.roman.romanNumeralFromChord for chord labeling.

    This is simpler than AugmentedNet but works without TensorFlow and
    provides a baseline for comparison.

    Returns a list of chord span dicts in the same format as
    _analyse_augmentednet().
    """
    try:
        import music21
    except ImportError:
        print('[WARN] music21 not installed -- returning empty analysis.')
        return []

    try:
        score = music21.converter.parse(score_path)
    except Exception as e:
        print(f'[WARN] music21 failed to parse {score_path}: {e}')
        return []

    spans: List[Dict[str, Any]] = []

    # Use chordify to get vertical sonorities
    try:
        chords = score.chordify()
    except Exception as e:
        print(f'[WARN] chordify failed for {score_path}: {e}')
        return []

    # Analyse key for the whole piece (fallback)
    try:
        global_key = score.analyze('key')
    except Exception:
        global_key = music21.key.Key('C')

    # Process each chord
    for chord_event in chords.recurse().getElementsByClass('Chord'):
        try:
            offset_ql = float(chord_event.offset)
            duration_ql = float(chord_event.duration.quarterLength)

            # Try to get a Roman numeral from the chord
            try:
                rn = music21.roman.romanNumeralFromChord(chord_event, global_key)
                roman_str = rn.romanNumeralAlone or str(rn.figure)
                quality = _classify_chord_quality(rn)
                root_pc = rn.root().pitchClass if rn.root() else 0
                inv = rn.inversion() if hasattr(rn, 'inversion') else 0
                is_secondary = '/' in str(rn.figure)
            except Exception:
                # If Roman numeral analysis fails, use basic chord info
                roman_str = chord_event.commonName or 'unknown'
                quality = _classify_basic_quality(chord_event)
                root_pc = chord_event.root().pitchClass if chord_event.root() else 0
                inv = chord_event.inversion() if hasattr(chord_event, 'inversion') else 0
                is_secondary = False

            spans.append({
                'onset_beat': offset_ql,
                'offset_beat': offset_ql + duration_ql,
                'roman': roman_str,
                'quality': quality,
                'root_pc': int(root_pc),
                'inversion': int(inv),
...
def _classify_chord_quality(rn) -> str:
    """Classify a music21 RomanNumeral into a quality string."""
    fig = str(rn.figure).lower()
    if 'dim7' in fig or 'o7' in fig:
        return 'dim7'
    if 'dim' in fig or 'o' in fig:
        return 'diminished'
    if 'aug' in fig or '+' in fig:
        return 'augmented'
    if '7' in fig:
        # Distinguish dominant 7th from minor 7th, major 7th
        quality = rn.quality if hasattr(rn, 'quality') else ''
        if quality == 'dominant' or (rn.romanNumeralAlone and rn.romanNumeralAlone[0].isupper() and '7' in fig):
            return 'dominant7'
        if quality == 'minor':
            return 'minor7'
        if quality == 'major':
        ...
        if quality == 'half-diminished':
            return 'halfDim7'
        return 'dominant7'  # default for unqualified 7ths
    quality = rn.quality if hasattr(rn, 'quality') else ''
    if quality == 'minor' or (rn.romanNumeralAlone and rn.romanNumeralAlone[0].islower()):
        return 'minor'
    return 'major'


def _classify_basic_quality(chord) -> str:
    """Classify a music21 Chord into a quality string (fallback)."""
    try:
        q = chord.quality
        if q == 'major':
            return 'major'
